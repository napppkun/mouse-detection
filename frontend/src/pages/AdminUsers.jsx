...
const RAW = window._env_?.BACKEND_URL || process.env.BACKEND_URL || "http://localhost:5000";
...
export default function AdminUsers() {
  ...
  async function grant(email, grant) {
    try {
      ...
      const res = await fetch(`${API}/api/admin/grant-admin`, {
        method: "POST",
      ...
      <div className="main-wrap">
        <div className="card">
          <div className="card-head">
            <h3 className="card-title">ผู้ใช้ทั้งหมด</h3>
            <input className="input" placeholder="ค้นหาอีเมล/ชื่อ…" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
          ...
          <div style={{ overflowX: "auto" }}>
            <table className="table">
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={5} className="muted">ไม่พบผู้ใช้</td>
                  </tr>
                )}
              </tbody>
            </table>
